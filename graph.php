...
$sql = "SELECT AVG(TEMPERATURE),avg(HUMIDITY), date(ID),dayofweek(date(ID)) FROM logdata GROUP BY DATE(ID) ORDER BY DATE(ID) DESC LIMIT 7;"; 
...
while($rows=$result->fetch_assoc()) 
    { 
        ...
        array_push($temp,$rows['AVG(TEMPERATURE)']);
        array_push($hum,$rows['avg(HUMIDITY)']);
        array_push($days,$dowMap[$rows['dayofweek(date(ID))']]); 
        // echo "<br>Status Fan: ".$rows['FAN']; 
    }
//echo $date[6];
$dataPoints1 = array(
